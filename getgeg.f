...
C        LAURENT MODIFICATION: REPLACED
C       CALL GETVAL(LINE(ISTART(3):),GEO(1,NATOMS),TGEO(1,NATOMS))
         CALL GETVALG(LINE(ISTART(3):),GEO(1,NATOMS),TGEO(1,NATOMS))
C       END LAURENT
         IF(NATOMS.EQ.2) GOTO 50
         NB(NATOMS)=READA(LINE,ISTART(4))
C        LAURENT MODIFICATION: REPLACED
C       CALL GETVAL(LINE(ISTART(5):),GEO(2,NATOMS),TGEO(2,NATOMS))
         CALL GETVALG(LINE(ISTART(5):),GEO(2,NATOMS),TGEO(2,NATOMS))
C       END LAURENT
         IF(NATOMS.EQ.3) GOTO 50
         NC(NATOMS)=READA(LINE,ISTART(6))
C        LAURENT MODIFICATION: REPLACED
C           CALL GETVAL(LINE(ISTART(7):),GEO(3,NATOMS),TGEO(3,NATOMS))
         CALL GETVALG(LINE(ISTART(7):),GEO(3,NATOMS),TGEO(3,NATOMS))
C         END LAURENT
   50    CONTINUE
   60 CONTINUE
   70 NATOMS=NATOMS-1
...
C#   28 WRITE(6,'(3I6)')LOCPAR(I),IDEPFN(I),LOCDEP(I)
      ...
      END
C     LAURENT MODIFICATION: REPLACED
C     SUBROUTINE GETVAL(LINE,X,T)
      SUBROUTINE GETVALG(LINE,X,T)
C     END LAURENT
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
