     ...
     2       /EULER / TVEC(3,3), ID
      ...
      LOGICAL GEOOK
C       LAURENT MODIFIATION
      COMMON /LINPEN / PAXIS,POFF,PI,PJ,PK,KF,PENRGY,PDIST,POW,KI
      INTEGER PI,PJ,PK
      DOUBLE PRECISION PAXIS(3),POFF(3),XYZ(3,NUMATM),LN,KF,
     1 PENRGY,PDIST,POW,SEP(3),KI
C       END LAURENT
...
C
   ...
   90 CONTINUE
C       LAURENT MODIFICATION: LINEARIZATION
      IF(PI.NE.O)THEN
      CALL SETLINPEN(COORD)
!      SEP(1)=COORD(1,PK)-POFF(1)
!      SEP(2)=COORD(2,PK)-POFF(2)
!      SEP(3)=COORD(3,PK)-POFF(3)
!      LN=DOT_PRODUCT(PAXIS,SEP)
!      COORD(1,PK)=POFF(1)+LN*PAXIS(1)
!      COORD(2,PK)=POFF(2)+LN*PAXIS(2)
!      COORD(3,PK)=POFF(3)+LN*PAXIS(3)
      ENDIF
C       END LAURENT
