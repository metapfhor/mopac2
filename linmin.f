...
C       LAURENT MODIFICATION
      ...
      INTEGER PI,PJ,PK
      DOUBLE PRECISION PAXIS(3),POFF(3),KF,PENERGY,PDIST,POWS
C       END LAURENT
...
C
   ...
   20 XMAXM=MAX(XMAXM,PABS)
     ...
     1CALL COMPFG(XPARAM, .TRUE., FUNCT,.TRUE.,GRAD,.FALSE.)
      ...
      DO 30 I=1,NVAR
C       LAURENT MODIFICATION
!       REMOVED   30 XPARAM(I)=XPAREF(I)+ALPHA*PVECT(I)
   30 IF(PI.EQ.0.OR.LOC(2,I).EQ.1.OR.(LOC(1,I).NE.PI.AND.LOC(1,I).NE.
     1PJ.AND.LOC(1,I).NE.PK))XPARAM(I)=XPAREF(I)+ALPHA*PVECT(I)
C       END LAURENT
...
C
      ...
      DO 40 I=1,NVAR
C       LAURENT MODIFICATION
!       REMOVED:   40 XPARAM(I)=XPAREF(I)+ALPHA*PVECT(I)
   40 IF(PI.EQ.0.OR.LOC(2,I).EQ.1.OR.(LOC(1,I).NE.PI.AND.LOC(1,I).NE.
     1PJ.AND.LOC(1,I).NE.PK))XPARAM(I)=XPAREF(I)+ALPHA*PVECT(I)
C       END LAURENT
...
C
  ...
  110    IF (ABS(ALPHA-VT(I)).LT.DELTA1*(1.D0+VT(I)).AND.OKF) GOTO 190
         DO 120 I=1,NVAR
C       LAURENT MODIFICATION
!       REMOVED:  120    XPARAM(I)=XPAREF(I)+ALPHA*PVECT(I)
!  120 IF(PI.EQ.0.OR.LOC(2,I).EQ.1.OR.(LOC(1,I).NE.PI.AND.LOC(1,I).NE.
!     1PJ.AND.LOC(1,I).NE.PK))XPARAM(I)=XPAREF(I)+ALPHA*PVECT(I)
  120 IF(PI.EQ.0.OR.LOC(2,I).EQ.1.OR.(LOC(1,I).NE.PI.AND.LOC(1,I).NE.
     1PJ.AND.LOC(1,I).NE.PK))XPARAM(I)=XPAREF(I)+ALPHA*PVECT(I)
C       END LAURENT
