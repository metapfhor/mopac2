...
C       LAURENT MODIFICATION
      ...
      INTEGER PI,PJ,PK
      DOUBLE PRECISION PAXIS(3),POFF(3),KF,PENRGY,PDIST,POW,KI
      COMMON /GENRAL/ COORD(3,NUMATM), COLD(3,NUMATM*3), GOLD(MAXPAR)
      COMMON /STFU/ DAXIS,XPAREF(MAXPAR)
...
C
  ...
  480    FORMAT(' RESTART FILE WRITTEN,   TIME LEFT:',F9.1,
     1' GRAD.:',F10.3,' HEAT:',G13.7)
      ...
      ELSE
C       LAURENT MODIFICATION
      CALL PENENERGY(COORD,FOO)
C       END LAURENT
         IF(MINPRT) WRITE(6,490)JCYC,MIN(TCYCLE,9999.99D0),
     1MIN(TLEFT,9999999.9D0),MIN(GNORM,999999.999D0),FUNCT1-PENRGY,
     2PDIST
C       LAURENT MODIFICATION
!        GMIN=MIN(GMIN,GNORM)
!        IF((GNORM.GT.KF.AND.(ABS(GNORM-LSTGRN)/GNORM.LT.5D-2.
!     1   OR.LSTGRN.EQ.0)).OR.(GNORM/KF).LT.1.D-3)THEN
!            KF=FLOOR(MIN(LOG10(GNORM),0.D0))+1.D0
!            KF=1.D1**(KF)
!C            GOTO 300
!        ENDIF
!        IF((GNORM-GMIN).GT.0.D0.AND.(GNORM-GMIN).LT.TOLERG*1.D1)THEN
!            STBL=.TRUE.
!            IF(STBL)THEN
!                KI=K*10.D0
!                KI=MIN(KI,1.D3)
...
!            STBL=.FALSE.
!        ENDIF
!      KF=MIN(MAX(KF,1.D1),KI)
!        LSTGRN=GNORM
      IF(DAXIS.GE.GNORM*2.D0.AND..NOT.STBL)THEN

            STBL=.TRUE.
            IF(LSTGRN.NE.0.D0)THEN
                KF=KF*5.D0
                GOTO 300
            ENDIF
        ENDIF
        LSTGRN=GNORM
        IF(.NOT.FAR.AND.PDIST.GT.1.D-1)THEN
            FAR=.TRUE.
            KF=KF*10.D0
            GOTO 300
        ENDIF
C       END LAURENT
  ...
  490    FORMAT(' CYCLE:',I4,' TIME:',F7.2,' TIME LEFT:',F9.1,
     1' GRAD.:',F10.3,' HEAT:',G13.7,' PENALTY:',G13.7)
      ENDIF
